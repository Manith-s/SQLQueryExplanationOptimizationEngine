    ...
    for s in suggestions:
        rows.append(
            [
                ...
                (
                    f"{float(s.get('estCostBefore')):.3f}"
                    if s.get("estCostBefore") is not None
                    else ""
                ),
                (
                    f"{float(s.get('estCostAfter')):.3f}"
                    if s.get("estCostAfter") is not None
                    else ""
                ),
                (
                    f"{float(s.get('estCostDelta')):.3f}"
                    if s.get("estCostDelta") is not None
                    else ""
                ),
            ]
        )
...
def cmd_optimize(args: argparse.Namespace) -> int:
    ...
    if args.what_if:
        try:
            wi = whatif.evaluate(
                sql, suggestions, timeout_ms=args.timeout_ms, force_enabled=True
            )
    ...
    if getattr(args, "diff", False) and (
        whatif_info.get("enabled") and whatif_info.get("available")
    ):
        try:
                        ...
                        with db.get_conn() as conn:
                            with conn.cursor() as cur:
                                ...
                                after = db.run_explain_costs(
                                    sql, timeout_ms=args.timeout_ms
                                )
                                cur.execute("SELECT hypopg_reset()")
...
def cmd_workload(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "markdown", False):
        ...
        for s in out["suggestions"]:
            print(
                f"- {s.get('title')} (score={s.get('score')}, freq={s.get('frequency')})"
            )
    elif getattr(args, "table", False):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    opt.add_argument(
        ...
        dest="what_if",
        action="store_true",
        help="Enable HypoPG cost-based what-if evaluation",
    )
    opt.add_argument("--no-what-if", dest="what_if", action="store_false")
    opt.add_argument(
        "--table",
        action="store_true",
        help="Print compact table of suggestions with cost deltas when available",
    )
    opt.add_argument(
        "--markdown", action="store_true", help="Print human-readable markdown report"
    )
    opt.add_argument(
        "--diff",
        action="store_true",
        help="Include plan diff for top suggestion when what-if ran",
    )
    opt.set_defaults(what_if=False)
    opt.set_defaults(func=cmd_optimize)

    wl = sp.add_parser(
        "workload", help="Analyze a file with multiple SQL statements (one per line)"
    )
