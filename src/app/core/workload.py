...
def _detect_patterns(
    sql: str, ast_info: Dict[str, Any], plan: Dict[str, Any] = None
) -> List[str]:
    """
...
def _merge_candidates(
    all_suggs: List[Dict[str, Any]], top_k: int
) -> List[Dict[str, Any]]:
    ...
    for s in all_suggs:
        ...
        cur["score"] = float(
            f"{(float(cur.get('score') or 0.0) + float(s.get('score') or 0.0)):.3f}"
        )
...
def analyze_workload(
    sqls: List[str], top_k: int = 10, what_if: bool = False
) -> Dict[str, Any]:
    """
    ...
    for sql in sqls:
        ...
        if info.get("type") != "SELECT":
            per_query.append(
                {"sql": sql, "skipped": True, "reason": "Non-SELECT statement"}
            )
            continue
        ...
        try:
            plan = db.run_explain(
                sql, analyze=False, timeout_ms=settings.OPT_TIMEOUT_MS_DEFAULT
            )
        ...
        try:
            tables = [
                t.get("name") for t in (info.get("tables") or []) if t.get("name")
            ]
            stats = db.fetch_table_stats(
                tables, timeout_ms=settings.OPT_TIMEOUT_MS_DEFAULT
            )
        except Exception:
            stats = {}
    ...
    grouped_queries = [
        ...
        for pattern_hash, queries in sorted(
            query_groups.items(), key=lambda x: -len(x[1])
        )[
            :20
        ]  # Top 20 groups
    ]
...
def _generate_workload_recommendations(
    ...
    recommendations.sort(
        key=lambda x: (
            impact_order.get(x.get("impact", "low"), 0),
            x.get("affectedQueries", 0),
        ),
        reverse=True,
    )
