...
async def get_catalog(
    ...
    try:
        ...
        if not schema_data or "tables" not in schema_data:
            raise HTTPException(
                status_code=404, detail=f"Schema '{schema}' not found or inaccessible"
            )
            ...
            enhanced_table = {
                **table,
                "statistics": statistics.get(
                    table_name, {"row_count": None, "total_size": "Unknown"}
                ),
            }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch catalog: {str(e)}"
        ) from e
...
async def validate_query(request: ValidateRequest):
    ...
    try:
        ...
        if (
            "DELETE FROM" in sql_upper or "UPDATE" in sql_upper
...
async def suggest_query(request: SuggestRequest):
    ...
    try:
        ...
        return SuggestResponse(
            suggestions=suggestions[:20], context_hints=context_hints  # Return top 20
        )
...
async def get_visual_plan(
    ...
    try:
        ...
        return VisualPlanResponse(
            plan_tree=plan_tree, summary=summary, bottlenecks=bottlenecks
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate visual plan: {str(e)}"
        ) from e
...
def _transform_plan_for_viz(node: Dict[str, Any], depth: int = 0) -> Dict[str, Any]:
    ...
    viz_node = {
        ...
        "cost": {"startup": node.get("Startup Cost"), "total": node.get("Total Cost")},
        "rows": {"plan": node.get("Plan Rows"), "actual": node.get("Actual Rows")},
        "time": {"actual": node.get("Actual Total Time")},
        "color": color,
        "icon": icon,
        "depth": depth,
        "children": [],
    ...
    if "Plans" in node:
        for child in node["Plans"]:
            viz_node["children"].append(_transform_plan_for_viz(child, depth + 1))
...
def _identify_bottlenecks(
    node: Dict[str, Any], bottlenecks: List = None
) -> List[Dict[str, Any]]:
