...
class AnalyzeRequest(BaseModel):
    ...
    tables: Optional[List[str]] = Field(
        None, description="Specific tables to analyze (or None for all)"
    )
...
class RecommendRequest(BaseModel):
    ...
    query_patterns: Optional[List[Dict[str, Any]]] = Field(
        None, description="Query patterns to analyze for recommendations"
    )
...
class AutoTuneRequest(BaseModel):
    ...
    auto_approve: bool = Field(
        default=False, description="Auto-approve low-risk changes"
    )
    ...
    try:
        ...
        if request.include_stats:
            ...
            tables_to_analyze = (
                request.tables
                if request.tables
                else [idx.table_name for idx in all_indexes[:5]]
            )
            ...
            for table in tables_to_analyze:
                ...
                if stats:
                    table_stats[table] = {
                        ...
                        "size_mb": float(
                            f"{stats[0].total_size_bytes / (1024*1024):.2f}"
                        ),
                        "dead_tuples": stats[0].n_dead_tup,
                        "last_analyze": (
                            stats[0].last_analyze.isoformat()
                            if stats[0].last_analyze
                            else None
                        ),
                    }
...
async def recommend_indexes(request: RecommendRequest):
    ...
    try:
        ...
        # Filter by priority
        filtered_recs = [
            r for r in recommendations if r.priority >= request.min_priority
        ]
        ...
        total_cost_mb = sum(r.estimated_cost_bytes for r in filtered_recs) / (
            1024 * 1024
        )
        ...
        recommendations_data = [
            {
                ...
                "estimated_cost_mb": float(
                    f"{rec.estimated_cost_bytes / (1024*1024):.2f}"
                ),
                "confidence": rec.confidence,
                "ddl": rec.to_ddl(request.schema),
            }
...
async def estimate_impact(request: ImpactRequest):
    ...
    try:
        ...
        using_clause = (
            f"USING {request.index_type.upper()}"
            if request.index_type != "btree"
            else ""
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Impact analysis failed: {str(e)}"
        ) from e
    ...
    try:
        ...
        if index_health["health_score"] < 70:
            recommendations.append(
                "Run index analysis to identify optimization opportunities"
            )
        if health_status["performance"]["severity"] != "ok":
            recommendations.append(
                "Performance degradation detected - consider automated optimization"
            )
        ...
        return HealthResponse(
            ...
            recommendations=(
                recommendations if recommendations else ["System health is good"]
            ),
...
async def enable_auto_tune(request: AutoTuneRequest, background_tasks: BackgroundTasks):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Auto-tune configuration failed: {str(e)}"
        ) from e
...
async def get_healing_actions(
    ...
    try:
        ...
        actions = healing_mgr.get_action_history(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve actions: {str(e)}"
        ) from e
...
async def rollback_action(action_id: str, schema: str = Query(default="public")):
    """
    ...
    try:
        ...
        return {"action_id": action_id, "rollback_result": result}
...
async def get_table_statistics(
    ...
    try:
        ...
        if include_columns:
            ...
            return {
                ...
                "statistics": {
                    ...
                    "size_mb": float(
                        f"{table_stats[0].total_size_bytes / (1024*1024):.2f}"
                    ),
                    "index_size_mb": float(
                        f"{table_stats[0].index_size_bytes / (1024*1024):.2f}"
                    ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Statistics collection failed: {str(e)}"
        ) from e
