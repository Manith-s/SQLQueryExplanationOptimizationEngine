...
def test_explain_nl_simple(client):
    ...
    response = client.post(
        "/api/v1/explain",
        json={"sql": "SELECT * FROM users", "nl": True, "analyze": False},
    )
...
def test_explain_nl_huge_plan(client):
    ...
    response = client.post(
        "/api/v1/explain",
        json={"sql": "SELECT 1", "nl": True, "analyze": False, "plan": huge_plan},
    )
...
def test_explain_nl_invalid_provider(client):
    ...
    response = client.post("/api/v1/explain", json={"sql": "SELECT 1", "nl": True})
